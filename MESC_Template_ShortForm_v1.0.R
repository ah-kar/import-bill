...
ui <- shinyUI(fluidPage(
  ...
  # App title ----
  titlePanel("CMS Bill Import Cleansing Portal- MESC_SHORT_FMT"),
...
server <- shinyServer(function(input, output) {
  ...
  df_products_upload <- reactive({
    ...
    Address	<- df$char5
    Bill_Code	<- df$char17
    df$char10 <- format(as.Date(df$char10, '%m/%d/%Y'), '%d/%m/%Y')
  ...
  output$downloadData <- downloadHandler(
    filename = function() {paste(input$billCode,"_20250413123102315.xlsx", sep="")},
