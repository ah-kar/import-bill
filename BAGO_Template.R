...
ui <- shinyUI(fluidPage(
  ...
  # App title ----
  titlePanel("CMS Bill Import Cleansing Portal- BGO_ESE|WSE"),
...
server <- shinyServer(function(input, output) {
  ...
  df_products_upload <- reactive({
    ...
    ##df <- df[-1,]
    #View(df)
    Ledger_No	<- paste(paste(df$char4,df$char5,sep = 'P/'),df$char6,sep = "/")
    Barcode	<- df$char2
    Meter_No <- df$char11
    Customer_Id <- df$char2
    Name	<- df$char7
    Address <- gsub("NA","",paste(df$char8,paste(df$char9,df$char10)))
    Bill_Code	<- df$char3
    #df$char14 <- format(as.Date(df$char14, '%m/%d/%Y'), '%d/%m/%Y')
    Due_Date <- ifelse(nchar(df$char14) == 10,
                       Due_Date <- df$char14,
                       Due_Date <- format(as.Date(as.numeric(df$char14),"1899-12-30"),"%d-%m-%Y"))
    Units	<- as.numeric(df$char12)
    Electrical_Fees	<- as.numeric(df$char13)	
    Service_Charges	<- 0.00
    Horse_Power_Fees	<- 0.00
    Discount	<- 0.00
    Last_Balance <-  0.00
    Total	<- as.numeric(df$char13)
    Debt	<- 0.00
    Fine	<- 0.00
    Grand_Total <- as.numeric(df$char13)
  ...
  output$downloadData <- downloadHandler(
    filename = function() {paste(input$billCode,"_20250414090842387.xlsx", sep="")},
